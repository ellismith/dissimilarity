#!/usr/bin/env python
"""
Visualize raw PC values vs age for top subtype-PC combinations
"""
...
print("="*70)
print("VISUALIZING RAW PC VALUES")
print("="*70)
...
for _, row in top_hits.iterrows():
    ...
    try:
        ...
        # --------------------------------------------------------------
        slope, intercept, r, pval, _ = stats.linregress(data['age'], data[pc])
        ...
        ax.set_title(
            ...
            f"{pc}: r={r:.3f}, p={pval:.2e} {sig_marker}",
...
plt.tight_layout()
plt.savefig(f'{output_dir}/raw_pc_values_top12.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"\nSaved: {output_dir}/raw_pc_values_top12.png")
print("="*70)
print("DONE!")
print("="*70)
