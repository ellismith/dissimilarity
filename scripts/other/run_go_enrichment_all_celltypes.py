...
for file in gene_files:
    ...
    # Format: gene_loadings_CellType_Region_Factor#_with_symbols.csv
    ...
    cell_type = parts[0]
    region = parts[1]
    factor = '_'.join(parts[2:])  # In case factor has underscores
    
    name = f"{cell_type}_{region}_{factor}"
...
if all_results:
    ...
    results_df.to_csv('go_enrichment_all_celltypes_results.csv', index=False)
    print(f"\n{'='*70}")
    print(f"Saved all results to: go_enrichment_all_celltypes_results.csv")
