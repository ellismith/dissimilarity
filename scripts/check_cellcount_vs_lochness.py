...
for cl in louvain_clusters:
    ...
    for idx in cell_indices:
        ...
        lochness.append(score)
        donors.append(donor)

    df = pd.DataFrame({"donor": donors, "lochness": lochness})
    agg = df.groupby("donor").agg(
        mean_lochness=("lochness", "mean"),
        cell_count=("lochness", "size")
    )

    if len(agg) < 5:
        continue

    rho, p = stats.spearmanr(agg["cell_count"], agg["mean_lochness"])
    results.append({"louvain": cl, "rho": rho, "p": p})

res_df = pd.DataFrame(results).sort_values("rho")
out = "/scratch/easmit31/factor_analysis/cellcount_vs_lochness_spearman.csv"
