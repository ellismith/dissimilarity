...
def analyze_celltype_region_pca(cell_type, adata_path, region):
    """Analyze one cell type in one region using PCA"""
    ...
    pc_scores = pca.fit_transform(pseudobulk_scaled)
    ...
    pca_df = pd.concat([animal_samples.reset_index(drop=True), pca_df], axis=1)
    
    # Correlate with age
    print("\n--- PC correlations with Age ---")
    best_corr = 0
    best_pc = None
    best_pval = 1
    best_pc_idx = None
    ...
    for i in range(n_components):
        pc_name = f'PC{i+1}'
        corr, pval = stats.pearsonr(pca_df[pc_name], pca_df['age'])
        ...
        if pval < 0.05:
            print(f"{pc_name}: r = {corr:.3f}, p = {pval:.3e}, var_exp = {var_exp:.1%} ***")
    
    print(f"\nBest age correlation: {best_pc} (r = {best_corr:.3f}, p = {best_pval:.3e})")
    
    # Save region-specific results
    pca_df.to_csv(f'pca_analysis_{cell_type}_{region}.csv', index=False)
    
    # Get gene loadings for best PC if significant
    if best_pval < 0.05:
        loadings = pca.components_
        pc_loadings = loadings[best_pc_idx]
        
        # Sort by absolute loading
        abs_sorted_idx = np.argsort(np.abs(pc_loadings))[::-1]
        
        # Create gene dataframe with symbols
        gene_df = pd.DataFrame({
            'gene': gene_names_filtered[abs_sorted_idx[:100]],
            'gene_symbol': [gene_mapping.get(g, g) for g in gene_names_filtered[abs_sorted_idx[:100]]],
            'loading': pc_loadings[abs_sorted_idx[:100]],
            'abs_loading': np.abs(pc_loadings[abs_sorted_idx[:100]])
        })
        
        gene_df.to_csv(f'pca_gene_loadings_{cell_type}_{region}_{best_pc}.csv', index=False)
        print(f"Saved gene loadings for {best_pc}")
    ...
    return {
        ...
        'best_pval': best_pval,
        'variance_explained': pca.explained_variance_ratio_[best_pc_idx] if best_pc_idx is not None else None
    }
...
for cell_type, adata_path in cell_types.items():
    ...
    for region in sorted(regions):
        try:
            result = analyze_celltype_region_pca(cell_type, adata_path, region)
            if result:
...
# Create summary
print(f"\n{'='*70}")
print("SUMMARY ACROSS ALL CELL TYPES (PCA)")
print(f"{'='*70}")
...
if all_results:
    ...
    print("\nTop age-associated PCs across all cell types:")
    print(sig_results[['cell_type', 'region', 'best_pc', 'best_age_corr', 'best_pval', 'variance_explained']].to_string(index=False))
    
    results_df.to_csv('pca_analysis_all_celltypes_summary.csv', index=False)
    sig_results.to_csv('pca_analysis_all_celltypes_significant.csv', index=False)
    print("\nSaved: pca_analysis_all_celltypes_summary.csv")
    print("Saved: pca_analysis_all_celltypes_significant.csv")
    ...
    for cell_type in sig_results['cell_type'].unique():
        ...
        print(f"  Mean variance explained by age-PC: {ct_df['variance_explained'].mean():.1%}")
