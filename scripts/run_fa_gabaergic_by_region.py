...
print("Loading GABAergic neuron data...")
adata = sc.read_h5ad(
    '/scratch/nsnyderm/u01/intermediate_files/cell-class_h5ad_update/Res1_GABAergic-neurons_subset.h5ad',
    backed='r'
)
...
for region in sorted(regions):
    ...
    fa_df.to_csv(f'factor_analysis_GABAergic_{region}.csv', index=False)

# Summary across regions
print(f"\n{'='*60}")
print("SUMMARY ACROSS REGIONS - GABAergic Neurons")
print(f"{'='*60}")
...
results_df.to_csv('factor_analysis_GABAergic_by_region_summary.csv', index=False)
print("\nSaved: factor_analysis_GABAergic_by_region_summary.csv")
...
plt.tight_layout()
plt.savefig('age_correlation_GABAergic_by_region.png', dpi=300, bbox_inches='tight')
print("Saved: age_correlation_GABAergic_by_region.png")

# Load astrocyte results for comparison
print("\n" + "="*60)
print("COMPARISON: Astrocytes vs GABAergic Neurons")
print("="*60)
astro_results = pd.read_csv('factor_analysis_by_region_summary.csv')

comparison = results_df.merge(astro_results, on='region', suffixes=('_gaba', '_astro'))
comparison = comparison[['region', 'best_age_corr_gaba', 'best_age_corr_astro']]
comparison['abs_diff'] = abs(comparison['best_age_corr_gaba']) - abs(comparison['best_age_corr_astro'])
comparison = comparison.sort_values('abs_diff', ascending=False)

print("\nRegions where GABAergic shows stronger age signal than Astrocytes:")
print(comparison[comparison['abs_diff'] > 0].to_string(index=False))

print("\nRegions where Astrocytes show stronger age signal than GABAergic:")
print(comparison[comparison['abs_diff'] < 0].to_string(index=False))

comparison.to_csv('astrocyte_vs_GABAergic_comparison.csv', index=False)
