...
print(f"Found {len(louvain_clusters)} louvain clusters in {args.region}")
...
# All cell types: h5ad filename and label
declare -a H5ADS=(
    ...
    "Res1_opc-olig_subset.h5ad:opc-olig"
...
N_CELL_TYPES=${#H5ADS[@]}   # 11
N_REGIONS=${#REGIONS[@]}    # 10 (excluding HIP for within-animal since already done... or just rerun)

# Map SLURM array index to cell type × region
CT_IDX=$(( SLURM_ARRAY_TASK_ID / ${#REGIONS[@]} ))
RG_IDX=$(( SLURM_ARRAY_TASK_ID % ${#REGIONS[@]} ))

IFS=':' read -r H5AD_FILE CELL_TYPE <<< "${H5ADS[$CT_IDX]}"
REGION="${REGIONS[$RG_IDX]}"

echo "Task ${SLURM_ARRAY_TASK_ID}: ${CELL_TYPE} × ${REGION}"
...
# Run Part 1 (within-animal centroid)
...
# Run Part 2 (population centroid)
