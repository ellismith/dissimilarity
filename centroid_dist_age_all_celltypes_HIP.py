...
for cell_type, fname in cell_type_files.items():
    ...
    for cl in louvain_clusters:
        ...
        agg = df.groupby(['animal_id', 'age'])['dist'].mean().reset_index().sort_values('age')
        if len(agg) < 5:
            continue
        ...
        ax.set_title(f'{cell_type} {region} louvain {cl}\nr={r:.2f}, p={p:.3f}, n={len(agg)} animals')
        plt.tight_layout()

        out = f'/scratch/easmit31/factor_analysis/{cell_type}_{region}_louvain{cl}_centroid_dist.png'
